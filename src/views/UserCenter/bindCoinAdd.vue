<template>
    <div class="w-full py-4 h-screen">
        <div class="flex text-center items-center px-4">
            <span class="px-3 text-lg text-gray-100 tracking-wide font-bold">{{t('bind_coin')}}</span>
        ...
        <div class="flex items-center px-7 py-3">
            ...
            <form @click.prevent class="py-5 px-2">
                <div class="flex text-center items-center relative">
                    <input autocomplete="off" name="coinAddress" type="text" v-model="coinAddress"
                        class="text-sm placeholder-gray-400 text-gray-300 pl-5 pr-10 border
                         border-gray-400 max-w-sm w-80 py-2 focus:outline-none focus:border-blue-400 bg-slate-800"
         ...
         <div class="flex items-center px-7 py-3">
            <button @click="bindCoinAdrr()" class="px-6 py-2 inline-flex  tracking-wider  bg-primary text-black shadow-lg rounded-md outline-none focus:outline-none ">Bind Address</button>
         </div>
    </div>
</template>
...
onMounted(() => {
  let str = '0x26C350De4507b1523A3a4088FAC7EC010C859632'
