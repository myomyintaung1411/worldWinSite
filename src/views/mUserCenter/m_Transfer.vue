<template>
  <div class="md:hidden relative max-h-screen h-screen overflow-y-scroll w-full bg-gray-900">
    <div class="sticky top-0 z- w-full py-2 px-2 flex items-center bg-slate-700">
      ...
      <span class="ml-6 text-gray-200 text-md tracking-wide font-bold">转账</span>
      ...
      <TabGroup>
        <TabList class="bg-gray-700 shadow-lg flex justify-center sticky top-0">
          <Tab v-slot="{ selected }" as="template" class="flex justify-center items-center">
            <div :class="[selected ? ' border-b-2 py-2  leading-tight border-emerald-500 text-white  ' : ' text-gray-400']" class="px-3 cursor-pointer font-bold">普通模式</div>
          </Tab>
          <Tab v-slot="{ selected }" as="template" class="flex justify-center items-center">
            <div :class="[selected ? ' border-b-2 py-2  leading-tight border-emerald-500 text-white  ' : ' text-gray-400']" class="px-3 cursor-pointer font-bold">免转模式</div>
          </Tab>
        </TabList>
