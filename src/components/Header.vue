    ...
    <div class="px-4 py-1 mx-auto sm:max-w-xl md:max-w-screen-md h-auto lg:max-w-screen-xl md:px-0 lg:px-8">
      <div class="relative flex items-center justify-between">
        ...
        <ul class="items-center hidden md:flex md:space-x-3 lg:space-x-8 lg:flex">
          <li>
            <router-link to="/" aria-label="Home" title="Home" class="px-4 py-2 font-medium tracking-wide text-gray-100 transition-colors duration-200 hover__class" :class="{ activeTab: activeRoute === '/home' }" @click="TabClick(1)">{{t('h')}}</router-link>
          </li>
          <li @mouseover="close(1)">
            <router-link to="/sport" aria-label="Our Sport" title="Our Sport" class="px-4 py-2 font-medium tracking-wide text-gray-100 transition-colors duration-200 hover__class " :class="{ activeTab: activeRoute === '/sport' }" @click="TabClick(2)">{{t('s')}}</router-link>
          </li>
          <li @mouseover="close(2)">
            <router-link to="/lottery" aria-label="Lottery" title="Lottery" @click="TabClick(3)" class="px-4 py-2 font-medium tracking-wide text-gray-100 transition-colors duration-200 hover__class" :class="{ activeTab: activeRoute === '/lottery'}">{{t('l')}}</router-link>
          </li>
          <li @mouseover="close(3)">
            <router-link to="/realbet" aria-label="Realbet" title="Realbet" @click="TabClick(4)" class="px-4 py-2 font-medium tracking-wide text-gray-100 transition-colors duration-200 hover__class" :class="{ activeTab: activeRoute === '/realbet' }">{{t('r')}}</router-link>
          </li>
          <li v-if="!lToken">
            <router-link to="/register" class="inline-flex items-center justify-center py-2 px-6 font-medium tracking-wide text-white transition duration-200 rounded shadow-lg bg-gray-900 hover:bg-primary hover:text-black hover:font-bold focus:shadow-outline focus:outline-none" aria-label="Sign up" title="Sign up">{{t('reg')}}</router-link>
          </li>
          <li v-if="!lToken">
            <router-link to="/login" class="inline-flex items-center justify-center py-2 px-6 font-medium tracking-wide text-white transition duration-200 rounded shadow-lg bg-gray-900 hover:bg-primary hover:text-black hover:font-bold focus:shadow-outline focus:outline-none" aria-label="Sign in" title="Sign in">{{t('login')}}</router-link>
          </li>
          <Menu v-if="lToken" as="div" class="ml-3 relative z-20">
            <div>
              <MenuButton class="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:focus:ring-primary">
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
              <MenuItems class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-gray-700 ring-1 ring-black ring-opacity-5 focus:outline-none">
                <MenuItem v-slot="{ active }">
                  <router-link to="/userCenter" :class="[active ? 'bg-gray-600 text-primary' : '', 'block px-4 py-2 text-sm text-gray-300']">Your Profile</router-link>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                  <div @click="SignOut" :class="[active ? 'bg-gray-600 text-primary' : '', 'block px-4 py-2 text-sm text-gray-300']">Sign out</div>
                </MenuItem>
              </MenuItems>
            </transition>
          </Menu>
            ...
            <div class="p-5 bg-gray-900 rounded shadow-sm h-auto">
              <div class="flex items-center justify-between mb-4">
                <div class="bg-white rounded-full">
                  <img src="@/assets/home/logo.png" alt="logo" draggable="false" class="w-auto h-16 text-center text-teal-accent-400" />
                </div>
              ...
              <nav>
                <ul @click="isMenuOpen = false" class="space-y-4">
                  <li>
                    <router-link to="/home" :class="activeRoute === '/home' ? 'text-primary' : 'text-gray-50'" aria-label="Home" title="Home" class="font-medium block hover:bg-gray-700 px-3 py-2 tracking-wide transition-colors duration-200">{{t('h')}}</router-link>
                  </li>
                  <li>
                    <router-link to="/sport" :class="activeRoute === '/sport' ? 'text-primary' : 'text-gray-50'" aria-label="Sport" title="Sport" class="font-medium tracking-wide block hover:bg-gray-700 px-3 py-2 transition-colors duration-200">{{t('s')}}</router-link>
                  </li>
                  <li>
                    <router-link to="/lottery" :class="activeRoute === '/lottery' ? 'text-primary' : 'text-gray-50'" aria-label="Lottery" title="Lottery" class="font-medium block tracking-wide hover:bg-gray-700 px-3 py-2 transition-colors duration-200">{{t('l')}}</router-link>
                  </li>
                  <li>
                    <router-link to="/realbet" :class="activeRoute === '/realbet' ? 'text-primary' : 'text-gray-50'" aria-label="Realbet" title="Realbet" class="font-medium tracking-wide block hover:bg-gray-700 px-3 py-2 transition-colors duration-200">{{t('r')}}</router-link>
                  </li>
                  <li v-if="!lToken">
                    <router-link to="/register" class="inline-flex items-center justify-center w-full h-12 px-6 font-medium tracking-wide transition duration-200 rounded shadow-md bg-primary text-black hover:bg-slate-500 focus:shadow-outline focus:outline-none" aria-label="Sign up" title="Sign up">{{t('reg')}}</router-link>
                  </li>
                  <li v-if="!lToken">
                    <router-link to="/login" class="inline-flex items-center justify-center w-full h-12 px-6 font-medium tracking-wide transition duration-200 rounded shadow-md bg-primary text-black hover:bg-slate-500 focus:shadow-outline focus:outline-none" aria-label="Sign in" title="Sign in">{{t('login')}}</router-link>
                  </li>
